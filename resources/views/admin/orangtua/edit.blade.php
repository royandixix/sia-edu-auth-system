...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Edit Orang Tua</h1>
            <p class="text-sm text-gray-500">Perbarui data orang tua</p>
        </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-900">Form Edit Orang Tua</h2>
        </div>

        <form action="{{ route('admin.orangtua.update', $orangtua->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="siswa_id"
                        ...
                        @foreach($siswa as $s)
                            <option value="{{ $s->id }}"
                                {{ $orangtua->siswa_id == $s->id ? 'selected' : '' }}>
                                {{ $s->nama_siswa }}
                            </option>
                        @endforeach
